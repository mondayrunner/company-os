...
test("stripe: monthly amount across intervals", () => {
  ...
  const s = normalizeSubscription({ id: "sub_1", status: "active", start_date: 1780000000, currency: "eur", customer: { name: "Koos", email: "k@x.nl" }, items: { data: [price(25000, "month")] } });
  assert.deepEqual([s.customer, s.monthly, s.currency, s.interval], ["Koos", 250, "EUR", "month"]);
});
...
test("imap: header parsing and encoded words", () => {
  const h = parseHeaders("Subject: =?utf-8?B?SGFsbG8gVGltIMOpw6k=?=\r\nFrom: \"Koos\" <koos@x.nl>\r\nDate: Mon, 3 Aug 2026 10:00:00 +0200\r\n");
  assert.equal(h.subject, "Hallo Tim éé");
