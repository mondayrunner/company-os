...
2. **Remembering.** Markdown in git is the canon. `brain.db` (SQLite, FTS5) is an index you can delete; `company-os index` rebuilds it. The one exception is metrics — a daily number cannot be recomputed once the day has passed — so every snapshot also writes the whole series to `metrics.csv`, which git tracks and `index` reads back. Choice: context sovereignty — the brain sits above the platforms and above the models, so no vendor can lock it in or throttle it.
