...
// Which sources make a name trustworthy enough to link on its own. A name that
// only ever appeared in a pipeline row or a calendar invite is a guess; a name
// that has a folder, a subscription or a task behind it is a fact. Both lists
// are config, because the labels come from whichever names module you plug in —
// they used to be hardcoded here, in the vocabulary of one private connector.
const strongSources = (ctx) => new Set(ctx.config.link?.strongSources ?? ["account-folder", "finance", "tasks", "stripe", "trello"]);
const weakSources = (ctx) => ctx.config.link?.weakSources ?? ["pipeline", "calendar"];
...
function scoreAccounts(ctx, text, index) {
  const t = text.toLowerCase();
  const strongSet = strongSources(ctx);
  const weak = weakSources(ctx);
  const per = new Map();
  for (const v of index.values()) {
    ...
    const strong = n.length >= 5 && [...sources].some((s) => strongSet.has(s)) && !weak.every((s) => sources.has(s));
...
export async function link(ctx, { dryRun = false } = {}) {
  ...
  for (const b of files) {
    ...
    out.seen++;
    const scores = scoreAccounts(ctx, body, index);
