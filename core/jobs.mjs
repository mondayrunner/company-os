...
// `brainlane jobs run` as a fallback when the job wrote none) and a log that
// rotates at 2 MB.
//
//   "scheduler": { "logs": "~/Library/Logs", "logPrefix": "brainlane-", "labelPrefix": "com.brainlane." },
//   "jobs": [
//     { "name": "index",  "title": "Index the vault", "run": "brainlane index", "cron": "45 7 * * 1-5" },
//     { "name": "check",  "title": "Weekly checks",   "run": "brainlane check", "cron": "0 8 * * 1", "label": "nl.example.check" },
//     { "name": "ui",     "title": "Dashboard",       "run": "brainlane ui",    "service": true }
//   ]
//
...
const DEFAULT_SCHEDULER = { logs: "~/.local/state/brainlane/logs", logPrefix: "", labelPrefix: "com.brainlane." };
...
function commandFor(ctx, job) {
  const bin = process.argv[1] && process.argv[1].endsWith("brainlane.mjs") ? `${process.execPath} ${process.argv[1]}` : "brainlane";
...
export function renderLaunchd(ctx, job) {
  const dict = {
    ...
    EnvironmentVariables: { HOME: ctx.home, PATH: pathFor(ctx), BRAINLANE_ROOT: ctx.root, ...(job.env ?? {}) },
...
/** Generate the files for a target into <root>/.brainlane/jobs/<target>/ and install them. */
...
export async function install(ctx, { target = detectTarget(), only = null, dryRun = false, force = false } = {}) {
  ...
  const outDir = join(ctx.root, ".brainlane", "jobs", target);
  ...
  if (target === "launchd") {
    ...
    if (!dryRun) {
      ...
      const stripped = current.replace(/# brainlane:begin[\s\S]*?# brainlane:end\n?/g, "").trimEnd();
...
export async function uninstall(ctx, { target = detectTarget(), only = null } = {}) {
  ...
  if (target === "cron") {
    const current = await exec("crontab", ["-l"]).then((r) => r.stdout).catch(() => "");
    const p = spawn("crontab", ["-"]); p.stdin.end(current.replace(/# brainlane:begin[\s\S]*?# brainlane:end\n?/g, ""));
...
export async function runJob(ctx, name) {
  ...
  const code = await new Promise((resolve) => {
    const p = spawn("/bin/bash", ["-lc", job.run], { cwd: ctx.root, stdio: "inherit", env: { ...process.env, BRAINLANE_ROOT: ctx.root, PATH: `${pathFor(ctx)}:${process.env.PATH ?? ""}`, ...(job.env ?? {}) } });
...
export async function listJobs(ctx) {
  ...
  for (const j of jobsOf(ctx)) {
    let status = null;
    try { status = JSON.parse(await readFile(join(ctx.stateDir, `${j.name}-status.json`), "utf8")); } catch {}
    ...
    out.push({ name: j.name, title: j.title ?? j.name, label: j.label, run: j.run, schedule: d.text, cron: j.cron ?? null, service: !!j.service, weekdays: d.weekdays, weekly: d.weekly, monthly: d.monthly, log: j.log,
      lastRun: status?.last_run ?? status?.laatste_run ?? null, result: status?.result ?? status?.resultaat ?? null, message: status?.message ?? status?.melding ?? null });
  }
  return out;
}
