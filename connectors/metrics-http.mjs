...
export default {
  ...
  async scan(ctx, options) {
    ...
    // A route that 404s or an expression that stopped matching used to drop its
    // metric without a word: the line simply stopped growing and nobody knew.
    return { metrics, count: metrics.length, message: `${metrics.length} metrics for ${date}${missing.length ? ` · no value for ${missing.join(", ")}` : ""}` };
  },
};
