...
```bash
cd ui && npm install
BRAINLANE_ROOT=/path/to/your/vault npm run dev     # http://localhost:4321
```

Or as a job: add `{ "name": "ui", "title": "Dashboard", "run": "cd ui && npx nuxt dev --port 4321", "service": true }` to `jobs` and run `brainlane jobs install`.

## It mirrors your config

There is no fixed set of panels. The overview shows one panel per live connector *kind* in your `brainlane.config.json`: a `finance` connector gives a finance panel, no `mail` connector means no mail panel. `/status` lists every connector and job with its state — that page *is* the module overview. Turning something off is one line in the config.
...
export default defineNuxtConfig({ extends: ["../brainlane/ui"] })
```
